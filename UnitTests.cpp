...
//
// A driver containing the zybooks unit testing code
...
bool testCount(ostream& testFeedback);
...
bool testDisplaySorted2(ostream& testFeedback);
...
int main(){
    ...
    cout << "Test display sorted movies 2: ";
    testDisplaySorted2(cout);
...
bool testDisplaySorted2(ostream& testFeedback){
    movieQueue = *movieQueueFromArray(titles2, N_TITLES);
    stringstream s, s1;
    movieQueue.displaySorted(s);
    movieQueue.display(s1);
    string expected = "0: Birdemic: Shock and Terror\n"
    ...
    if (passed){
        ...
        if (str != expected)
            testFeedback << "Expected sorted 2: " << endl << expected << endl << endl << "Got:" << endl << str << endl;
...
bool testCount(ostream& testFeedback) {
    movieQueue = *movieQueueFromArray(titles, N_TITLES);
    int count = movieQueue.count();
    int expected = 5;

    if (count != expected){
        cout << "Expected movie count: " << expected << endl
             << "Got: " << count << endl;
        cout << "Failed" << endl;
        return false;
    }
